	...
	const signUpMutation = useMutation({
		...
		onError: (error) => {
			...
			if (errorData?.errors?.fplTeamId) {
				setErrors((prev) => ({
					...prev,
					fplTeamId: errorData.errors?.fplTeamId,
				}));
			}
			if (errorData?.errors?.fplLeagueId) {
				setErrors((prev) => ({
					...prev,
					fplLeagueId: errorData.errors?.fplLeagueId,
				}));
			}
		},
	});
				...
				<Form onSubmit={handleSubmit} className="space-y-4">
					...
					<Field>
						...
						<FieldControl
							...
							aria-describedby={
								errors.fplTeamId ? errorIds.fplTeamId : undefined
							}
						/>
						{errors.fplTeamId && (
							<FieldError id={errorIds.fplTeamId}>
								{errors.fplTeamId}
							</FieldError>
						)}
					</Field>
					...
					<Field>
						<FieldLabel htmlFor={fieldIds.fplLeagueId}>
							FPL League ID
						</FieldLabel>
						<FieldControl
						...
						/>
						{errors.fplLeagueId && (
							<FieldError id={errorIds.fplLeagueId}>
								{errors.fplLeagueId}
							</FieldError>
						)}
					</Field>

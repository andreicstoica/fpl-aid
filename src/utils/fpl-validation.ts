...
export async function validateFplTeamId(
	...
	try {
		...
		return {
			valid: false,
			error:
				"Network error while validating team ID. Please check your connection and try again.",
		};
	}
}
 ...
 */
export async function validateFplLeagueId(
	leagueId: string,
): Promise<FplValidationResult> {
	try {
		// Convert to number to validate format
		const numericLeagueId = parseInt(leagueId, 10);
		if (Number.isNaN(numericLeagueId) || numericLeagueId <= 0) {
			return {
				valid: false,
		...
		const response = await fetch(
			`${FPL_API_BASE}/leagues-classic/${numericLeagueId}/standings/`,
		);
		...
		return {
			valid: false,
			error:
				"Network error while validating league ID. Please check your connection and try again.",
		};
	}
}
 ...
 */
export async function validateFplData(
	teamId: string,
	leagueId: string,
): Promise<{

...
export const validatePassword = (password: string, minLength: number = 0): string | undefined => {
  if (!password.trim()) {
...
export const validateFplId = (id: string, fieldName: string): string | undefined => {
  if (!id.trim()) {
...
export const validateSignUpForm = (formData: SignUpFormData): ValidationErrors<SignUpFormData> => {
  ...
  const favoriteTeamError = validateRequired(formData.favoriteTeam, "Favorite team")
...
export const validateSignInForm = (formData: SignInFormData): ValidationErrors<SignInFormData> => {
